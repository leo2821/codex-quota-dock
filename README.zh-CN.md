...
应用启动后会保存本机 Codex 已登录的 ChatGPT 账号，并查询该账号的额度。顶部菜单栏显示 `Codex Quota Dock` 和当前账号的剩余额度，点击后可查看所有已保存账号。
...
- `AccountCore`：账号、额度、钥匙串、文件保存、界面语言与 Codex 通信。
...
1.1.0 发布包已经在 macOS 26.6.2、Apple Silicon、Codex CLI `0.155.0-alpha.2.6` 上完成编译、启动和 34 项集成检查。三个真实账号均已成功刷新额度。英文账号页面、添加账号入口、切换确认和两种语言设置已经通过 macOS 辅助功能接口检查，重新启动后继续使用所选英文界面。160 条翻译通过资源完整性与格式参数检查。两个不同真实账号之间的完整桌面切换尚未完成验证，当前版本以预发布形式提供。
